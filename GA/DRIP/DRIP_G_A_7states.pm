...
sub mate{  
	...
	while(scalar @{$self->{INDIVIDUALS}}<$main::POPULATION_SIZE){
		...
		if (rand(1)<$main::CROSSOVER_RATE){
			...
			my $ind= Individual->new();
			$ind->mate($self->{INDIVIDUALS}->[$father_iter], $self->{INDIVIDUALS}->[$mother_iter]); #NOTE ???
			$ind->mutate();
			push @{$self->{INDIVIDUALS}},$ind;
		}
	}
...
#      print "$element\n";
#    }
		my $comm = "faster_better_eval_peaks.pl $res $threshold"; # NOTE FIXME need this script!
		push(@totalQ, $comm);
	}
