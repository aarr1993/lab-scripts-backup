...
void split_files (char * wig, char ** beds, int num_beds, char ** chromosomes, int num_chrs, char **** bedfiles, char *** wigfiles, int *** bedlinecount) ; 
void process_bed (BedLine **** bedlines, char *** bedfiles, int num_beds, char ** chromosomes, int num_chrs, int ** bedlinecount) ;
...
int main (int argc, char * argv[]) 
{
  ...
  split_files(wig, beds, num, chromosomes, num_chrs, &bedfiles, &wigfiles, &bedlinecount) ;  
  BedLine *** bedlines ; // the messed up array

  process_bed (&bedlines, bedfiles, num, chromosomes, num_chrs, bedlinecount) ;
//process_bed -> below array, bedfiles, chroms, num_chrs, num
  // (chr) array of (input bed)array of array of STRUCTS -> **** again
      // or return ptr to this messed up array -> ***
      // no, need to declare the struct thing here in main
      // This is going to be HORRIBLE (malloc everything struct* every line)
//process_wig
//get_values
...
// can't initialize BedLine * because it is the array of separate line BedLines
// this needs to be malloced EVERY LINE
// unless there is a way to get a line-count before hand.
// maybe in split_files keep a count of everything in a messed-up array?
// for now I will malloc, later consider this // NO DO IT NOW
...
//        printf("%s\n", ptr) ;
        ...
        printf("start : %d end : %d strand : %s\n", (*bedlines)[j][n][size].start, (*bedlines)[j][n][size].end, (*bedlines)[j][n][size].strand) ;
        size++ ;
//        if (curr_size != 0)
//        {
          // THIS IS A BAD IDEA
//          (*bedlines)[j][n] = (BedLine *) malloc (size * sizeof (BedLine) ) ; 
//        }
